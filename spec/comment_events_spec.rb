...
RSpec.describe 'Comment assessments' do
  ...
  let(:decision) { JSON.generate(labels: [], reply: 'provider', sources: []) }
  ...
  it 'does not turn fresh measurements into another diagnostic questionnaire' do
    ...
    allow(assessment).to receive(:ask_copilot).and_return(
      JSON.generate(labels: [], reply: nil, sources: [])
    )
  ...
  it 'does not suppress an agent-selected clarification based on wording' do
    comment['body'] = 'CPU usage is now 98% focused and 96% unfocused.'
  ...
  it 'supplies earlier questions so the agent can avoid reworded repetition' do
  ...
  it 'allows a similar question about a different version' do
    ...
    allow(assessment).to receive(:ask_copilot).and_return(
      JSON.generate(labels: [], reply: nil, sources: [], comment: 'Does restarting the app fix this in version 1.2.4?')
    )
  ...
  it 'preserves a new diagnostic command even with otherwise identical wording' do
    ...
    allow(assessment).to receive(:ask_copilot).and_return(
      JSON.generate(labels: [], reply: nil, sources: [], comment: 'What output do you get from `app --help`?')
    )
